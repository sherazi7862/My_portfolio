...
export function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://formspree.io/f/mbdnjgey', {
        method: 'POST',
    ...
    <section id="contact" className="mx-auto max-w-7xl scroll-mt-24 px-6 py-28 md:px-10">
      <Reveal>
        <div className="mx-auto max-w-4xl rounded-3xl border border-border bg-surface p-8 text-center md:p-14">
          ...
          {status === 'sent' ? (
            <div className="mx-auto mt-10 max-w-2xl rounded-xl border border-green-500/30 bg-green-500/10 px-6 py-8 text-center">
              <CheckCircle2 className="mx-auto h-10 w-10 text-green-400" />
              <p className="mt-3 font-medium text-green-300">Message sent successfully!</p>
            ...
            >
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-primary to-[#2563eb] px-6 py-3.5 font-mono text-sm text-primary-foreground shadow-[0_10px_34px_-8px_rgba(61,107,255,0.55)] transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {status === 'sending' ? (
                  <>
